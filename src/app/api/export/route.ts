import { NextResponse } from 'next/server'
import { db, isDemoMode } from '@/lib/firebase'
import { collection, getDocs } from 'firebase/firestore'
import * as XLSX from 'xlsx'

export async function GET() {
  try {
    let checkins: any[] = []

    if (isDemoMode) {
      // Get demo checkins from the checkin API
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/checkin`, {
        method: 'GET',
      })
      if (response.ok) {
        checkins = await response.json()
      }
    } else {
      const querySnapshot = await getDocs(collection(db, 'checkins'))
      checkins = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      }))
    }

    const data = checkins.map(checkin => ({
      'Full Name': checkin.fullName,
      'Centre Number': checkin.centreNumber,
      'Phone Number': checkin.phoneNumber,
      'Children': checkin.children?.map(child => `${child.name} (${child.age})`).join('; ') || '',
      'Timestamp': checkin.timestamp ? new Date(checkin.timestamp).toLocaleString() : new Date().toLocaleString(),
    }))

    const ws = XLSX.utils.json_to_sheet(data)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Checkins')

    const buffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })

    return new NextResponse(buffer, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': 'attachment; filename=checkins.xlsx',
      },
    })
  } catch (error) {
    console.error('Error exporting to Excel:', error)
    return NextResponse.json({ error: 'Failed to export' }, { status: 500 })
  }
}